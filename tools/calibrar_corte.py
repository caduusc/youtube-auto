...
from pipeline.trim import build_plan, normalize  # noqa: E402
...
def main(path: Path) -> None:
    ...
    print("  " + "-" * 62)
    if not candidatos:
        print("  Nenhum. O Whisper tende a limpar disfluencia na transcricao,")
        print("  entao o som pode existir no audio e nao aparecer no texto.")
        print("  Nesse caso o corte de hesitacao nao tem o que cortar, e o")
        print("  unico lever que sobra e `pause_min_seconds`.")
    else:
        print(f"  {'token':>10}  {'dur':>5}  {'sil.antes':>9}  {'sil.depois':>10}  veredito")
        for token, dur, antes, depois, quando in sorted(candidatos, key=lambda c: -c[1])[:20]:
            corta = (dur >= rules.filler_min_seconds
                     and min(antes, depois) >= rules.filler_silence_seconds)
            print(f"  {token:>10}  {dur:>4.2f}s  {antes:>8.2f}s  {depois:>9.2f}s  "
                  f"{'CORTA' if corta else 'mantem'}  ({quando:.1f}s)")
        cortados = sum(
            1 for _, d, a, p, _ in candidatos
            if d >= rules.filler_min_seconds and min(a, p) >= rules.filler_silence_seconds
        )
        print(f"\n  {len(candidatos)} candidatos, {cortados} passariam os limiares atuais")
        print("  Os que ficam em 'mantem' sao, em geral, o verbo 'e' ou o artigo")
        print("  'um' em fala corrida — cortar aqueles destruiria a frase.")
